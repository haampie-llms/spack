...
class NetworkClient:
    ...
    def _connection(self, name: str, build: Callable[[], _R]) -> _R:
    ...
    def ssl_context(self) -> ssl.SSLContext:
        ...
        return self._connection("ssl_context", lambda: _verifying_ssl_context(self.ssl_certs))

    @property
    def urlopen(self) -> OpenType:
        """Function with the signature of ``OpenerDirector.open``, whose ``timeout`` defaults to
        ``connect_timeout``."""
        return self._connection("urlopen", lambda: _build_opener(self))

    @property
    def oci_urlopen(self) -> "spack.oci.opener.OpenType":
        """Like ``urlopen``, with OCI authentication from the credentials of ``mirrors``."""

        def build():
            import spack.oci.opener

            return with_default_timeout(spack.oci.opener.create_opener(self), self.connect_timeout)

        return self._connection("oci_urlopen", build)

    @property
    def s3_clients(self) -> Dict["spack.util.s3.S3ClientKey", Any]:
        """S3 clients, keyed by the arguments they are created with."""
        return self._connection("s3_clients", dict)
