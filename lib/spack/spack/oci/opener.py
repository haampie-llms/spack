...
def oci_urlopen(client: spack.util.web.NetworkClient) -> OpenType:
    """The ``urlopen`` of ``client``, with OCI authentication from the credentials of its mirrors.
    It is built once per client and process."""
    return client.connection(
        "oci_urlopen",
        lambda: spack.util.web.with_default_timeout(create_opener(client), client.connect_timeout),
    )
