...
def deprecate(parser, args, ctx):
    ...
    for dcate, dcator in zip(all_deprecate, all_deprecators):
        deprecate_spec(dcate, dcator, symlink, ctx)


def deprecate_spec(
    spec: spack.spec.Spec, deprecator: spack.spec.Spec, link_fn, ctx: spack.context.SpackContext
) -> None:
    """Deprecate ``spec`` in favor of ``deprecator``, in the store of ``ctx``"""
    store = ctx.store
    ...
    for deprecated in store.db.specs_deprecated_by(spec):
        deprecate_spec(deprecated, deprecator, link_fn, ctx)
