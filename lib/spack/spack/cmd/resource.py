...
def _show_patch(sha256, repo: spack.repo.RepoPath):
    """Show a record from the patch index."""
    patches = repo.get_patch_index().index
    ...
    for package, rec in data.items():
        ...
        if "relative_path" in rec:
            pkg_dir = repo.get_pkg_class(owner).package_dir
...
def resource_list(args, ctx):
    """list all resources known to spack (currently just patches)"""
    patches = ctx.repo.get_patch_index().index
    for sha256 in patches:
        if args.only_hashes:
            print(sha256)
        else:
            _show_patch(sha256, ctx.repo)


def resource_show(args, ctx):
    """show a resource, identified by its checksum"""
    _show_patch(args.hash, ctx.repo)


def resource(parser, args, ctx):
    action = {"list": resource_list, "show": resource_show}
    action[args.resource_command](args, ctx)
