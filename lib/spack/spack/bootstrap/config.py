...
def bootstrap_config(ctx: "spack.context.SpackContext") -> spack.config.Configuration:
    """Return the configuration to bootstrap Spack's own dependencies with, derived from ``ctx``.

    It has the default and bootstrap scopes, plus the ``bootstrap``, ``config`` and ``repos``
    sections of ``ctx``. Software is installed in the bootstrap store, and the interpreter running
    Spack is the only Python.
    """
    user = ctx.config
