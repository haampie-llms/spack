...
def test_spack_entry_point_extension(
    ...
    extensions = spack.extensions.get_extension_paths(ctx.config)
    ...
    if not found:
        raise ValueError("Did not find extension in %s" % ", ".join(extensions))
    root = spack.extensions.load_extension("myext", ctx.config)
    assert os.path.samefile(root, my_ext)
    module = spack.extensions.get_module("spam", ctx.config)
