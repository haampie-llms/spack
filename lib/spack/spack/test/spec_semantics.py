...
class TestSpecSemantics:
    ...
    def test_propagation_conflicts_with_node_in_closure(self, lhs, rhs):
        """A propagated bool value contradicting a bool variant on a dependency denotes the
        empty set, but the spec operations look at one node at a time, map by map, and leave
        the contradiction to the concretizer: neither side is inside the other, yet they
        intersect and merge."""
        lhs, rhs = Spec(lhs), Spec(rhs)
        assert not lhs.satisfies(rhs)
        assert not rhs.satisfies(lhs)
