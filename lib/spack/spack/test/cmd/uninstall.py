...
@pytest.mark.db
def test_recursive_uninstall(mutable_database, ctx: SpackContext):
...
def test_uninstall_spec_with_multiple_roots(
    constraint, expected_number_of_specs, mutable_database, ctx: SpackContext
):
...
@pytest.mark.db
def test_force_uninstall_and_reinstall_by_hash(mutable_database, ctx: SpackContext):
