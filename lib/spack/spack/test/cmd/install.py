...
def test_package_output(install_mockery, mock_fetch, ctx: SpackContext):
    """
    ...
    spec = spack.concretize.concretize_one("printing-package", ctx)
    pkg = spec.package
...
def test_install_commit(
    ...
    spec = spack.concretize.concretize_one(f"git-test-commit@{commits[-1]}", ctx)
...
def test_install_overwrite_multiple(
    ...
    libdwarf = spack.concretize.concretize_one("libdwarf", ctx)
    cmake = spack.concretize.concretize_one("cmake", ctx)
...
def test_install_mix_cli_and_files(
    spec_format, clispecs, filespecs, tmp_path: pathlib.Path, ctx: SpackContext
):
    args = clispecs
...
def test_extra_files_are_archived(
    ...
    s = spack.concretize.concretize_one("archive-files", ctx)
...
def test_cdash_upload_build_error(
    capfd, tmp_path: pathlib.Path, mock_fetch, install_mockery, monkeypatch
):
...
def test_cdash_install_from_spec_json(
    ...
    with fs.working_dir(str(tmp_path)):
        ...
        pkg_spec = spack.concretize.concretize_one("pkg-c", ctx)
...
def test_cache_only_fails(mock_fetch, temporary_store: Store, install_mockery):
    # libelf from cache fails to install, which automatically removes the
    # the libdwarf build task
    install("--cache-only", "libdwarf", fail_on_error=False)
    assert isinstance(install.error, spack.error.InstallError)
    assert not temporary_store.db.query_local("libdwarf")
    assert not temporary_store.db.query_local("libelf")


def test_install_only_dependencies(mock_fetch, install_mockery, ctx: SpackContext):
...
def test_install_only_package(mock_fetch, install_mockery):
    ...
    assert "package only mode" in msg
    assert "dependency-install" in msg


def test_install_deps_then_package(mock_fetch, install_mockery, ctx: SpackContext):
    dep = spack.concretize.concretize_one("dependency-install", ctx)
    root = spack.concretize.concretize_one("dependent-install", ctx)
...
def test_install_only_dependencies_in_env(
    mutable_mock_env_path, mock_fetch, install_mockery, ctx: SpackContext
):
    env("create", "test")
...
def test_cdash_configure_warning(
    tmp_path: pathlib.Path, mock_fetch, install_mockery, ctx: SpackContext
):
    with fs.working_dir(str(tmp_path)):
        ...
        spec = spack.concretize.concretize_one("configure-warning", ctx)
...
def test_install_use_buildcache(
    ...
    def validate(mode, out, pkg):
        def from_cache(pkg, out):
            return any(f" {pkg}@" in ln and "from build cache" in ln for ln in out.splitlines())

        def assert_auto(pkg, out):
            assert from_cache(pkg, out)

        def assert_only(pkg, out):
            assert from_cache(pkg, out)

        def assert_never(pkg, out):
            assert not from_cache(pkg, out)
...
def test_concurrent_packages_set_in_config(
