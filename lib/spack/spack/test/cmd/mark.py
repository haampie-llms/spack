...
@pytest.mark.db
def test_mark_all_explicit(mutable_database: spack.database.Database, ctx: SpackContext):
    mark("-e", "-a")
    gc("-y")
...
@pytest.mark.db
def test_mark_all_implicit(mutable_database: spack.database.Database, ctx: SpackContext):
    mark("-i", "-a")
    gc("-y")
    all_specs = ctx.store.layout.all_specs()
...
@pytest.mark.db
def test_mark_one_explicit(mutable_database: spack.database.Database, ctx: SpackContext):
    mark("-e", "libelf")
...
@pytest.mark.db
def test_mark_one_implicit(mutable_database: spack.database.Database, ctx: SpackContext):
    mark("-i", "externaltest")
    gc("-y")
    all_specs = ctx.store.layout.all_specs()
...
@pytest.mark.db
def test_mark_all_implicit_then_explicit(
