...
@pytest.fixture()
def environment_from_manifest(tmp_path: pathlib.Path, ctx: SpackContext):
    """Returns a new environment named 'test' from the content of a manifest file."""

    def _create(content):
        spack_yaml = tmp_path / ev.manifest_name
        spack_yaml.write_text(content)
        return _env_create("test", init_file=str(spack_yaml), ctx=ctx)
...
def test_env_track_existing_env_fails(ctx: SpackContext):
    ...
    with pytest.raises(spack.main.SpackCommandError):
        env(
            ...
            "--name",
            "track_test",
            ev.environment_dir_from_name("track_test", config=ctx.config),
        )
...
def test_env_untrack_when_active(tmp_path: pathlib.Path, ctx: SpackContext):
    ...
    with fs.working_dir(str(tmp_path)):
        ...
        active_env = ev.read(env_name, ctx=ctx)
        with active_env:
...
def installed_environment(
    ...
    def _installed_environment(content):
        ...
        with fs.working_dir(tmp_path):
            ...
            with ev.read("test", ctx=ctx) as current_environment:
...
def test_add(ctx: SpackContext):
    e = ev.create("test", ctx=ctx)
    e.add("mpileaks")
    assert Spec("mpileaks") in e.user_specs


def test_change_match_spec(ctx: SpackContext):
    env("create", "test")

    e = ev.read("test", ctx=ctx)
    with e:
        add("mpileaks@2.1")
        add("mpileaks@2.2")
...
def test_change_multiple_matches(ctx: SpackContext):
    env("create", "test")
...
def test_env_add_virtual(ctx: SpackContext):
    env("create", "test")
    e = ev.read("test", ctx=ctx)
    e.add("mpi")
    e.concretize()
...
def test_env_add_nonexistent_fails(ctx: SpackContext):
    env("create", "test")
...
def test_env_remove(ctx: SpackContext):
    env("create", "foo")
    env("create", "bar")
...
def test_env_rename_managed(ctx: SpackContext):
...
def test_env_rename_independent(tmp_path: pathlib.Path, ctx: SpackContext):
    # Need real environment
    with pytest.raises(spack.main.SpackCommandError):
    ...
    assert not ev.is_env_dir(anon_foo, config=ctx.config)
    assert ev.is_env_dir(anon_bar, config=ctx.config)
    ...
    assert ev.is_env_dir(anon_bar, config=ctx.config)
    assert not ev.is_env_dir(anon_baz, config=ctx.config)
    ...
    assert errmsg in env.output
    assert ev.is_env_dir(anon_bar, config=ctx.config)
    assert ev.is_env_dir(anon_baz, config=ctx.config)

    env("rename", "-f", "-d", anon_bar, anon_baz)
    assert not ev.is_env_dir(anon_bar, config=ctx.config)
    assert ev.is_env_dir(anon_baz, config=ctx.config)
    ...
    qux.mkdir()
    anon_qux = str(qux)
    assert not ev.is_env_dir(anon_qux, config=ctx.config)
    ...
    assert not ev.is_env_dir(anon_baz, config=ctx.config)
    assert ev.is_env_dir(anon_qux, config=ctx.config)


def test_concretize(ctx: SpackContext):
    e = ev.create("test", ctx=ctx)
    e.add("mpileaks")
    e.concretize()
...
def test_env_specs_partition(install_mockery, mock_fetch, ctx: SpackContext):
    e = ev.create("test", ctx=ctx)
    e.add("cmake-client")
    e.concretize()
...
def test_env_install_all(temporary_store: Store, install_mockery, mock_fetch, ctx: SpackContext):
    e = ev.create("test", ctx=ctx)
    e.add("cmake-client")
    e.concretize()
    e.install_all(fake=True)
...
def test_env_install_single_spec(install_mockery, mock_fetch, ctx: SpackContext):
    env("create", "test")
    install = SpackCommand("install")
...
def test_env_roots_marked_explicit(
    temporary_store: Store, install_mockery, mock_fetch, ctx: SpackContext
):
    install = SpackCommand("install")
...
def test_env_modifications_error_on_activate(
    install_mockery, mock_fetch, monkeypatch, capfd, mock_packages: RepoPath, ctx: SpackContext
):
    env("create", "test")
    install = SpackCommand("install")
...
def test_env_definition_symlink(
    install_mockery, mock_fetch, tmp_path: pathlib.Path, ctx: SpackContext
):
...
def test_remove_after_concretize(ctx: SpackContext):
    e = ev.create("test", ctx=ctx)
...
def test_remove_before_concretize(mutable_config, ctx: SpackContext):
...
def test_remove_command(ctx: SpackContext):
    env("create", "test")
...
def test_remove_command_all(ctx: SpackContext):
...
def test_bad_remove_included_env(ctx: SpackContext):
    env("create", "test")
    test = ev.read("test", ctx=ctx)
...
def test_force_remove_included_env(ctx: SpackContext):
    env("create", "test")
    test = ev.read("test", ctx=ctx)
...
def test_environment_status(tmp_path: pathlib.Path, monkeypatch, ctx: SpackContext):
    with fs.working_dir(str(tmp_path)):
        assert "No active environment" in env("status")

        with ev.create("test", ctx=ctx):
            assert "In environment test" in env("status")

        with ev.create_in_dir("local_dir", ctx=ctx):
            assert os.path.join(os.getcwd(), "local_dir") in env("status")

            e = ev.create_in_dir("myproject", ctx=ctx)
            e.write()
...
def test_env_status_broken_view(
    ...
    with ev.create_in_dir(tmp_path, ctx=ctx):
...
def test_user_removed_spec(environment_from_manifest, ctx: SpackContext):
...
"""
    ...
    after = ev.read("test", ctx=ctx)
    after.concretize()
    after.write()

    read = ev.read("test", ctx=ctx)
    assert not any(x.name == "hypre" for x in read.all_specs_generator())


def test_lockfile_spliced_specs(environment_from_manifest, install_mockery, ctx: SpackContext):
    ...
    zmpi = spack.concretize.concretize_one("zmpi", ctx)
...
"""
    ...
    e2 = _env_create("test2", init_file=e1.lock_path, ctx=ctx)
...
def test_init_from_lockfile(environment_from_manifest, ctx: SpackContext):
...
def test_init_from_yaml(environment_from_manifest, ctx: SpackContext):
    """Test that an environment can be instantiated from a lockfile."""
    e1 = environment_from_manifest(
        """
spack:
...
"""
    ...
    e2 = _env_create("test2", init_file=e1.manifest_path, ctx=ctx)
...
def test_init_from_env(
    use_name, environment_from_manifest, mutable_config: Configuration, ctx: SpackContext
):
...
"""
    ...
    e2 = _env_create("test2", init_file="test" if use_name else e1.path, ctx=ctx)
...
def test_init_from_env_no_spackfile(tmp_path, ctx: SpackContext):
    with pytest.raises(ev.SpackEnvironmentError, match="not a valid environment"):
        _env_create("test", init_file=str(tmp_path), ctx=ctx)


def test_init_from_yaml_relative_includes(tmp_path: pathlib.Path, ctx: SpackContext):
    files = [
...
"""
    ...
    e2 = _env_create("test2", init_file=str(e1_manifest), ctx=ctx)
...
def test_init_from_yaml_relative_includes_outside_env(tmp_path: pathlib.Path, ctx: SpackContext):
...
"""
    ...
    with pytest.raises(ValueError, match="does not exist"):
        _ = _env_create("test2", init_file=str(e1_manifest), ctx=ctx)
...
def test_init_with_file_and_remove(tmp_path: pathlib.Path, monkeypatch, ctx: SpackContext):
    """Ensure a user can remove from any position in the spack.yaml file."""
    path = tmp_path / "spack.yaml"
...
def test_with_config_bad_include_activate(
    environment_from_manifest, tmp_path: pathlib.Path, ctx: SpackContext
):
...
"""
    ...
    with ev.Environment(env_root, ctx=ctx) as e:
        e.concretize()
...
def test_env_with_included_config_file(mutable_mock_env_path, packages_file, ctx: SpackContext):
...
"""
    ...
    e = ev.Environment(env_root, ctx=ctx)
    with e:
        e.concretize()
...
def test_config_change_existing(
    mutable_mock_env_path, tmp_path: pathlib.Path, mock_packages, mutable_config, ctx: SpackContext
):
...
"""
    ...
    with e:
        ...
        test_spec = spack.concretize.concretize_one("mpich", ctx)
        ...
        spack.concretize.concretize_one("libelf@0.8.12", ctx)
        ...
        test_spec = spack.concretize.concretize_one("fftw", ctx)
        ...
        spack.concretize.concretize_one("bowtie@1.3.0", ctx)
        spack.concretize.concretize_one("bowtie@1.2.2", ctx)
        # confirm that we cannot concretize to the old value
        with pytest.raises(spack.solver.asp.UnsatisfiableSpecError):
            spack.concretize.concretize_one("bowtie@1.2.0", ctx)
...
"""
    ...
    with ev.Environment(tmp_path, ctx=ctx):
        config("change", "packages:mpich:require:~debug")
        with pytest.raises(spack.solver.asp.UnsatisfiableSpecError):
            spack.concretize.concretize_one("mpich+debug", ctx)
        spack.concretize.concretize_one("mpich~debug", ctx)
...
"""
    ...
    with ev.Environment(tmp_path, ctx=ctx):
        assert spack.concretize.concretize_one("mpich", ctx).satisfies("@3.0.3")
        with pytest.raises(spack.error.ConfigError, match="not a list"):
            config("change", "packages:mpich:require:~debug")


def test_env_with_included_config_file_url(
    tmp_path: pathlib.Path, mutable_empty_config, packages_file, ctx: SpackContext
):
...
def test_env_with_included_config_scope(mutable_mock_env_path, packages_file, ctx: SpackContext):
...
def test_env_with_included_config_var_path(
    ...
    config_real_path = substitute_path_variables(config_var_path, ctx.config)
...
def test_env_with_included_config_precedence(tmp_path: pathlib.Path, ctx: SpackContext):
...
"""
    ...
    e = ev.Environment(tmp_path, ctx=ctx)
    with e:
        e.concretize()
...
def test_env_with_included_configs_precedence(tmp_path: pathlib.Path, ctx: SpackContext):
...
def test_env_loads(install_mockery, mock_fetch, mock_modules_root, ctx: SpackContext):
    env("create", "test")
...
def test_stage(mock_stage, mock_fetch, install_mockery, ctx: SpackContext):
    env("create", "test")
    with ev.read("test", ctx=ctx):
        add("mpileaks")
        add("zmpi")
...
def test_env_blocks_uninstall(mock_stage, mock_fetch, install_mockery, ctx: SpackContext):
    env("create", "test")
    with ev.read("test", ctx=ctx):
        add("mpileaks")
        install("--fake")
...
def test_roots_display_with_variants(ctx: SpackContext):
    env("create", "test")
    with ev.read("test", ctx=ctx):
        add("boost+shared")
...
def test_uninstall_keeps_in_env(
    mock_stage, mock_fetch, temporary_store: Store, install_mockery, ctx: SpackContext
):
...
def test_uninstall_removes_from_env(mock_stage, mock_fetch, install_mockery, ctx: SpackContext):
...
def test_indirect_build_dep(repo_builder: RepoBuilder, ctx: SpackContext):
    ...
    with spack.test.harness.use_repositories(ctx, repo_builder.root):
        ...
        _env_create("test", with_view=False, ctx=ctx)
        e = ev.read("test", ctx=ctx)
        e.add(x_spec)
        e.concretize()
        e.write()

        e_read = ev.read("test", ctx=ctx)
...
def test_store_different_build_deps(repo_builder: RepoBuilder, ctx: SpackContext):
...
def test_env_updates_view_install(
    tmp_path: pathlib.Path, mock_stage, mock_fetch, install_mockery, ctx: SpackContext
):
    view_dir = tmp_path / "view"
...
def test_env_include_concrete_env_yaml(env_name, ctx: SpackContext):
    env("create", "test")
    test = ev.read("test", ctx=ctx)
...
def test_env_include_concrete_old_env(format, ctx: SpackContext):
    ...
    assert ev.read("old-env", ctx=ctx).all_specs() == ev.read("test", ctx=ctx).all_specs()
...
def test_env_not_concrete_include_concrete_env(ctx: SpackContext):
    env("create", "test")
    test = ev.read("test", ctx=ctx)
...
def test_env_include_concrete_reuse(reuse_mode, ctx: SpackContext):
...
def test_concretize_nested_include_concrete_envs(ctx: SpackContext):
    env("create", "test1")
    test1 = ev.read("test1", ctx=ctx)
    with test1:
        add("zlib")
...
def test_concretize_nested_included_concrete(ctx: SpackContext):
...
def test_env_config_view_default(
    environment_from_manifest, mock_stage, mock_fetch, install_mockery, ctx: SpackContext
):
...
"""
    ...
    assert os.path.isdir(os.path.join(e.default_view.view(ctx.store)._root, ".spack", "mpileaks"))
...
def test_stack_yaml_definitions(tmp_path: pathlib.Path, ctx: SpackContext):
...
def test_stack_yaml_definitions_as_constraints(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_yaml_definitions_as_constraints_on_matrix(
...
def test_stack_yaml_definitions_write_reference(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_yaml_add_to_list(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_yaml_remove_from_list(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_yaml_remove_from_list_force(tmp_path: pathlib.Path, ctx: SpackContext):
    spack_yaml = tmp_path / ev.manifest_name
    spack_yaml.write_text(
        """\
spack:
...
def test_stack_yaml_remove_from_matrix_no_effect(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_yaml_force_remove_from_matrix(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_extension(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_conditional_false(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_conditional_true(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_conditional_with_variable(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_conditional_with_satisfaction(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_complex_conditional(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
def test_stack_definition_conditional_add_write(tmp_path: pathlib.Path, ctx: SpackContext):
    filename = str(tmp_path / "spack.yaml")
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            """\
...
spack:
  ...
  view:
    ...
    with ev.Environment(envdir, ctx=ctx):
        install("--fake")
    ...
    default:
      ...
      link_type: {link_type}"""
        ...
        for spec in test.roots():
            ...
            bin_file = pathlib.Path(test.default_view.view(ctx.store)._root) / "bin" / spec.name
...
def test_envvar_set_in_activate(
    tmp_path: pathlib.Path, mock_packages, install_mockery, ctx: SpackContext
):
    spack_yaml = tmp_path / "spack.yaml"
...
def test_env_activate_default_view_root_unconditional(mutable_mock_env_path, ctx: SpackContext):
...
def test_concretize_user_specs_together(mutable_config, ctx: SpackContext):
    with ev.create("coconcretization", ctx=ctx) as e:
        mutable_config.set("concretizer:unify", True)
...
def test_duplicate_packages_raise_when_concretizing_together(mutable_config, ctx: SpackContext):
    with ev.create("coconcretization", ctx=ctx) as e:
        mutable_config.set("concretizer:unify", True)
        e.add("mpileaks+opt")
        e.add("mpileaks~opt")
...
def test_env_write_only_non_default(ctx: SpackContext):
    env("create", "test")
    ...
    assert yaml == ev.default_manifest_yaml(ctx.config)


@pytest.mark.regression("20526")
def test_env_write_only_non_default_nested(tmp_path: pathlib.Path, ctx: SpackContext):
...
def test_newline_in_commented_sequence_is_not_an_issue(tmp_path: pathlib.Path, ctx: SpackContext):
    spack_yaml = """
spack:
  specs:
  - dyninst
...
def test_lockfile_not_deleted_on_write_error(
...
def test_rewrite_rel_dev_path_new_dir(tmp_path: pathlib.Path, ctx: SpackContext):
    ...
    with ev.Environment(str(dest_env), ctx=ctx) as e:
        assert e.dev_specs["mypkg1"]["path"] == str(build_folder)
        assert e.dev_specs["mypkg2"]["path"] == sep + os.path.join("some", "other", "path")


def test_rewrite_rel_dev_path_named_env(tmp_path: pathlib.Path, ctx: SpackContext):
    ...
    with ev.read("named_env", ctx=ctx) as e:
        assert e.dev_specs["mypkg1"]["path"] == str(build_folder)
        assert e.dev_specs["mypkg2"]["path"] == sep + os.path.join("some", "other", "path")


def test_does_not_rewrite_rel_dev_path_when_keep_relative_is_set(
    tmp_path: pathlib.Path, ctx: SpackContext
):
...
def test_custom_version_concretize_together(mutable_config, ctx: SpackContext):
    # Custom versions should be permitted in specs when
    # concretizing together
    with ev.create("custom_version", ctx=ctx) as e:
        mutable_config.set("concretizer:unify", True)
        # Concretize a first time using 'mpich' as the MPI provider
...
def test_modules_relative_to_views(
    environment_from_manifest, install_mockery, mock_fetch, ctx: SpackContext
):
    environment_from_manifest(
        """
spack:
...
"""
    ...
    with ev.read("test", ctx=ctx) as e:
        ...
        view_prefix = e.default_view.get_projection_for_spec(spec, ctx.config)
        ...
        for module_set in ("uses_view", "without_view"):
            ...
            for spec in specs:
                ...
                if module_set == "uses_view":
                    assert e.default_view.get_projection_for_spec(spec, ctx.config) in contents
                    assert spec.prefix not in contents
                else:
                    assert e.default_view.get_projection_for_spec(spec, ctx.config) not in contents
...
def test_virtual_spec_concretize_together(mutable_config, ctx: SpackContext):
    # An environment should permit to concretize "mpi"
    with ev.create("virtual_spec", ctx=ctx) as e:
        mutable_config.set("concretizer:unify", True)
        e.add("mpi")
        e.concretize()
...
def test_concretize_transactional(
    unify, method_to_fail, monkeypatch, mutable_config: Configuration, ctx: SpackContext
):
    mutable_config.set("concretizer:unify", unify)
    e = ev.create("test", ctx=ctx)
...
def test_query_develop_specs(tmp_path: pathlib.Path, ctx: SpackContext):
...
def test_custom_store_in_environment(mutable_config, tmp_path: pathlib.Path, ctx: SpackContext):
    spack_yaml = tmp_path / "spack.yaml"
...
def test_activate_temp(monkeypatch, tmp_path: pathlib.Path, ctx: SpackContext):
    ...
    assert ev.is_env_dir(str(tmp_path), config=ctx.config)
...
def test_create_and_activate_independent(tmp_path: pathlib.Path, ctx: SpackContext):
    with fs.working_dir(str(tmp_path)):
        ...
        assert ev.is_env_dir(env_dir, config=ctx.config)
        env("deactivate")


def test_activate_default(monkeypatch, ctx: SpackContext):
    """Tests whether `spack env activate` creates / activates the default
    environment"""
    assert not ev.exists("default", config=ctx.config)

    # Activating it the first time should create it
    env("activate", "--sh")
    env("deactivate", "--sh")
    assert ev.exists("default", config=ctx.config)
...
def test_env_view_fail_if_symlink_points_elsewhere(
    ...
    with ev.create("env", with_view=view, ctx=ctx):
        add("libelf")
        install("--fake")
...
def test_env_view_backward_compat_old_symlink_format(
    ...
    with ev.read("test", ctx=ctx) as e:
        ...
        specs = view_desc.specs_for_view(e.concrete_roots(), ctx.store, ctx.repo)
...
def test_failed_view_cleanup(
    tmp_path: pathlib.Path, mock_stage, mock_fetch, install_mockery, ctx: SpackContext
):
    """Tests whether Spack cleans up after itself when a view fails to create"""
    view_dir = tmp_path / "view"
    with ev.create("env", with_view=str(view_dir), ctx=ctx):
        add("libelf")
        install("--fake")
...
def test_read_old_lock_and_write_new(tmp_path: pathlib.Path, lockfile, ctx: SpackContext):
    ...
    _env_create("test", init_file=test_lockfile_path, with_view=False, ctx=ctx)
...
def test_read_v1_lock_creates_backup(tmp_path: pathlib.Path, ctx: SpackContext):
    ...
    e = ev.create_in_dir(tmp_path, init_file=test_lockfile_path, ctx=ctx)
...
def test_read_legacy_lockfile_and_reconcretize(
    mock_stage, mock_fetch, install_mockery, lockfile, tmp_path: pathlib.Path, ctx: SpackContext
):
...
def test_environment_depfile_makefile(
    depfile_flags, expected_installs, tmp_path: pathlib.Path, mock_packages, ctx: SpackContext
):
    env("create", "test")
...
def test_depfile_works_with_gitversions(
    tmp_path: pathlib.Path, mock_packages, monkeypatch, ctx: SpackContext
):
...
def test_depfile_phony_convenience_targets(
    ...
    make = Executable("make")
    with fs.working_dir(str(tmp_path)):
        with ev.create_in_dir(".", ctx=ctx):
            add("dttop")
            concretize()

        with ev.Environment(".", ctx=ctx) as e:
...
def test_environment_depfile_out(tmp_path: pathlib.Path, mock_packages, ctx: SpackContext):
    env("create", "test")
...
def test_spack_package_ids_variable(tmp_path: pathlib.Path, mock_packages, ctx: SpackContext):
...
"""
    ...
    with ev.read("test", ctx=ctx) as test:
        for s in test.all_specs():
            assert "post-install: {}".format(s.dag_hash()) in out


def test_depfile_empty_does_not_error(tmp_path: pathlib.Path, ctx: SpackContext):
...
def test_unify_when_possible_works_around_conflicts(mutable_config, ctx: SpackContext):
    with ev.create("coconcretization", ctx=ctx) as e:
...
def test_relative_view_path_on_command_line_is_made_absolute(
    tmp_path: pathlib.Path, ctx: SpackContext
):
    with fs.working_dir(str(tmp_path)):
        env("create", "--with-view", "view", "--dir", "env")
        environment = ev.Environment(os.path.join(".", "env"), ctx=ctx)
        environment.regenerate_views()
        assert os.path.samefile("view", environment.default_view.root)


def test_environment_created_in_users_location(
    mutable_mock_env_path, tmp_path: pathlib.Path, ctx: SpackContext
):
    ...
    assert env_dir in ev.root(dir_name, config=ctx.config)
    assert os.path.isdir(os.path.join(env_dir, dir_name))


def test_environment_created_from_lockfile_has_view(
    mock_packages, temporary_store, tmp_path: pathlib.Path, ctx: SpackContext
):
    ...
    env("create", "-d", env_a)
    with ev.Environment(env_a, ctx=ctx):
        add("libelf")
        install("--fake")
    ...
    with ev.Environment(env_b, ctx=ctx) as e:
        assert os.path.isdir(e.view_path_default)


def test_env_view_disabled(tmp_path: pathlib.Path, mutable_mock_env_path, ctx: SpackContext):
...
"""
    ...
    with ev.read("disabled", ctx=ctx) as e:
        e.concretize()
...
def test_env_include_mixed_views(
    tmp_path: pathlib.Path, mutable_config, mutable_mock_env_path, first, ctx: SpackContext
):
...
def test_env_view_resolves_identical_file_conflicts(
    ...
    with ev.create("env", with_view=tmp_path / "view", ctx=ctx) as e:
...
def test_concretized_specs_and_include_concrete(mutable_config, ctx: SpackContext):
    ...
    with ev.read("included-env", ctx=ctx) as e:
        e.add("mpileaks")
        e.add("libelf")
        e.add("pkg-a")
        mutable_config.set(
    ...
    with ev.read("main-env", ctx=ctx) as e:
        e.add("mpileaks")
        e.add("libelf@0.8.12")
        e.add("pkg-a")
...
"""
        ...
        for item in test.concretized_roots:
            # Assertions are based on the behavior of the "--fake" install
            bin_file = (
                pathlib.Path(test.default_view.view(ctx.store)._root) / "bin" / item.root.name
            )
...
def test_env_include_concrete_only(tmp_path, mock_packages, mutable_config, ctx: SpackContext):
...
"""
    ...
    include_env = ev.create("test_include", include_manifest, ctx=ctx)
...
def test_env_include_concrete_relative_path(
    tmp_path, mock_packages, mutable_config, ctx: SpackContext
):
...
"""
    ...
    with ev.Environment(str(include_dir), ctx=ctx) as e:
        e.concretize()
        e.write()
...
"""
    ...
    with ev.Environment(str(main_dir), ctx=ctx) as e:
        e.concretize()
        e.write()
        assert len(e.user_specs) == 0
...
def test_env_include_concrete_git_lockfile(
    tmp_path, mock_packages, mutable_config, monkeypatch, ctx: SpackContext
):
...
def test_create_with_orphaned_directory(mutable_mock_env_path: pathlib.Path, ctx: SpackContext):
    ...
    assert not ev.exists("test1", config=ctx.config)

    # Creating an environment over an orphaned directory must succeed
    env("create", "test1")

    assert ev.exists("test1", config=ctx.config)
...
def test_exists_consistent_with_all_environment_names(
    mutable_mock_env_path: pathlib.Path, setup: str, ctx: SpackContext
):
    ...
    if setup == "valid":
        manifest.write_text(ev.default_manifest_yaml(ctx.config))
    elif setup == "orphaned":
        pass  # no manifest
    elif setup == "broken_symlink":
        manifest.symlink_to("/nonexistent/spack.yaml")

    listed = "myenv" in ev.all_environment_names(ctx.config)
    assert ev.exists("myenv", config=ctx.config) == listed
...
def test_reported_total_ignores_specs_that_were_kept(
