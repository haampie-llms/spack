...
def test_env_write_only_non_default(ctx: SpackContext):
    ...
    assert yaml == ev.default_manifest_yaml(config=ctx.config)
...
def test_exists_consistent_with_all_environment_names(
    ...
    if setup == "valid":
        manifest.write_text(ev.default_manifest_yaml(config=ctx.config))
    elif setup == "orphaned":
        pass  # no manifest
    elif setup == "broken_symlink":
        manifest.symlink_to("/nonexistent/spack.yaml")

    listed = "myenv" in ev.all_environment_names(config=ctx.config)
