...
def test_default_variant(config, mock_packages):
    spec = spack.concretize.concretize_one("optional-dep-test-3", spack.context.current())
    assert "pkg-a" in spec

    spec = spack.concretize.concretize_one("optional-dep-test-3~var", spack.context.current())
    assert "pkg-a" in spec
