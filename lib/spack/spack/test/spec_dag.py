...
def test_test_deptype(repo_builder: RepoBuilder):
    ...
    with spack.repo.use_repositories(repo_builder.root):
        spec = spack.concretize.concretize_one("w", spack.context.current(), tests=("w",))
        assert "x" in spec
...
def test_installed_deps(monkeypatch, install_mockery):
    ...
    c_spec = spack.concretize.concretize_one(c, spack.context.current())
    ...
    a_spec = spack.concretize.concretize_one(f"{a} ^/{c_spec.dag_hash()}", spack.context.current())
...
def test_specify_preinstalled_dep(monkeypatch, repo_builder: RepoBuilder):
    ...
    with spack.repo.use_repositories(repo_builder.root):
        b_spec = spack.concretize.concretize_one("pkg-b", spack.context.current())
        monkeypatch.setattr(
        ...
        a_spec = spack.concretize.concretize_one(a_spec, spack.context.current())
...
class TestSpecDag:
    ...
    def test_traversal(self, pairs, traverse_kwargs):
        ...
        """
        dag = spack.concretize.concretize_one("mpileaks ^zmpi", spack.context.current())
    ...
    def test_dependents_and_dependencies_are_correct(self):
        ...
        check_links(spec)
        concrete = spack.concretize.concretize_one(spec, spack.context.current())
    ...
    def test_copy_concretized(self):
        orig = spack.concretize.concretize_one("mpileaks", spack.context.current())
        copy = orig.copy()
    ...
    def test_deptype_traversal(self, spec_str, deptypes, expected):
        dag = spack.concretize.concretize_one(spec_str, spack.context.current())
    ...
    def test_concretize_deptypes(self):
        ...
        s = spack.concretize.concretize_one("dt-diamond", spack.context.current())
        self.check_diamond_deptypes(s)

    def test_copy_deptypes(self):
        """Ensure that dependency types are preserved by spec copy."""
        s1 = spack.concretize.concretize_one("dt-diamond", spack.context.current())
...
def test_synthetic_construction_of_split_dependencies_from_same_package(mock_packages, config):
    ...
    root = spack.concretize.concretize_one("pkg-b", spack.context.current())
    link_run_spec = spack.concretize.concretize_one("pkg-c@=1.0", spack.context.current())
    build_spec = spack.concretize.concretize_one("pkg-c@=2.0", spack.context.current())
...
def test_synthetic_construction_bootstrapping(mock_packages, config):
    ...
    #  pkg-b@1.0
    #
    root = spack.concretize.concretize_one("pkg-b@=2.0", spack.context.current())
    bootstrap = spack.concretize.concretize_one("pkg-b@=1.0", spack.context.current())
...
def test_adding_overlapping_deptypes_to_different_versions_stays_parallel(
    ...
    p = spack.concretize.concretize_one("pkg-b@=2.0", spack.context.current())
    c1 = spack.concretize.concretize_one("pkg-b@=1.0", spack.context.current())
    c2 = spack.concretize.concretize_one("pkg-b@=2.0", spack.context.current())
...
def test_merging_a_concrete_direct_dep_keeps_its_dependent_edges(mock_packages, config):
    ...
    s = Spec("%pkg-b")
    t = Spec("")
    concrete = spack.concretize.concretize_one("pkg-b@=1.0", spack.context.current())
