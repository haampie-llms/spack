...
def test_test_log_name(mock_packages, config, ctx: SpackContext):
...
def test_test_ensure_stage(mock_test_stage, mock_packages, ctx: SpackContext):
...
def test_test_stage_caches(mock_packages, install_mockery, mock_test_stage, ctx: SpackContext):
...
def test_test_spec_passes(
    mock_packages, install_mockery, mock_test_stage, monkeypatch, ctx: SpackContext
):
...
def test_get_test_suite_too_many(mock_packages, mock_test_stage, ctx: SpackContext):
    test_suites = []
...
def test_test_function_names(
    mock_packages, install_mockery, virtuals, expected, ctx: SpackContext
):
...
def test_test_functions_pkgless(
    mock_packages, install_mockery, ensure_debug, capfd, ctx: SpackContext
):
...
def test_test_part_fail(
    tmp_path: pathlib.Path, install_mockery, mock_fetch, mock_test_stage, ctx: SpackContext
):
    """Confirm test_part with a ProcessError results in FAILED status."""
    s = spack.concretize.concretize_one("trivial-smoke-test", ctx)
    pkg = s.package
...
def test_test_part_pass(install_mockery, mock_fetch, mock_test_stage, ctx: SpackContext):
...
def test_test_part_skip(install_mockery, mock_fetch, mock_test_stage, ctx: SpackContext):
...
def test_embedded_test_part_status(
    install_mockery, mock_fetch, mock_test_stage, current, substatuses, expected, ctx: SpackContext
):
