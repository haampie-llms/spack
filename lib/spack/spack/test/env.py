...
class TestDirectoryInitialization:
    def test_environment_dir_from_name(self, mutable_mock_env_path):
        ...
        environment_dir = ev.environment_dir_from_name("test", config=spack.config.CONFIG)
        assert env.path == environment_dir
        with pytest.raises(ev.SpackEnvironmentError, match="environment already exists"):
            ev.environment_dir_from_name("test", exists_ok=False, config=spack.config.CONFIG)
    ...
    def test_environment_dir_from_nested_name(self, mutable_mock_env_path):
        ...
        env = ev.create("group/test", ctx=spack.context.current())
        environment_dir = ev.environment_dir_from_name("group/test", config=spack.config.CONFIG)
        assert env.path == environment_dir
        with pytest.raises(ev.SpackEnvironmentError, match="environment already exists"):
            ev.environment_dir_from_name("group/test", exists_ok=False, config=spack.config.CONFIG)
...
def test_hash_change_no_rehash_concrete(tmp_path: pathlib.Path, config):
    ...
    env = ev.create_in_dir(env_path, ctx=spack.context.current())
    env.write()
    ...
    # Read environment
    read_in = ev.Environment(env_path, ctx=spack.context.current())
...
def test_env_change_spec(tmp_path: pathlib.Path, config):
    ...
    env.change_existing_spec(change_spec, repo=spack.repo.PATH)
...
def test_env_change_spec_in_definition(tmp_path: pathlib.Path, mutable_mock_env_path):
    ...
    with e:
        e.change_existing_spec(
            spack.spec.Spec("mpileaks@2.2"), list_name="desired_specs", repo=spack.repo.PATH
        )
    e.write()
...
def test_env_change_spec_in_matrix_raises_error(tmp_path: pathlib.Path, mutable_mock_env_path):
    ...
    with pytest.raises(ev.SpackEnvironmentError) as error:
        e.change_existing_spec(spack.spec.Spec("mpileaks@2.2"), repo=spack.repo.PATH)
...
def test_user_view_path_is_not_canonicalized_in_yaml(tmp_path: pathlib.Path, config):
    ...
    with fs.working_dir(str(tmp_path)):
        fst = ev.create_in_dir(env_path, with_view=view, ctx=spack.context.current())
    ...
    with fs.working_dir(str(tmp_path)):
        snd = ev.Environment(env_path, ctx=spack.context.current())
 ...
 """
    ...
    with fs.working_dir(str(tmp_path)):
        with pytest.raises(ev.SpackEnvironmentError):
            e = ev.Environment(str(tmp_path), ctx=spack.context.current())
            e.activate()
...
def test_adding_anonymous_specs_to_env_fails(tmp_path: pathlib.Path):
    ...
    raises an exception
    """
    env = ev.create_in_dir(tmp_path, ctx=spack.context.current())
...
def test_update_default_view(init_view, update_value, tmp_path: pathlib.Path, config):
    ...
    env = ev.create_in_dir(tmp_path, with_view=init_view, ctx=spack.context.current())
...
def test_update_default_complex_view(
    ...
    env = ev.Environment(tmp_path, ctx=spack.context.current())
    env.update_default_view(update_value)
    env.write(regenerate=True)
...
def test_cannot_initialize_in_dir_with_init_file(tmp_path: pathlib.Path, filename):
    ...
    with pytest.raises(ev.SpackEnvironmentError, match="cannot initialize"):
        ev.create_in_dir(tmp_path, ctx=spack.context.current())
...
def test_cannot_initiliaze_if_dirname_exists_as_a_file(tmp_path: pathlib.Path):
    ...
    dir_name.touch()
    with pytest.raises(ev.SpackEnvironmentError, match="cannot initialize"):
        ev.create_in_dir(dir_name, ctx=spack.context.current())
...
def test_cannot_initialize_if_init_file_does_not_exist(tmp_path: pathlib.Path):
    ...
    with pytest.raises(ev.SpackEnvironmentError, match="cannot initialize"):
        ev.create_in_dir(tmp_path, init_file=init_file, ctx=spack.context.current())


def test_environment_pickle(tmp_path: pathlib.Path):
    env1 = ev.create_in_dir(tmp_path, ctx=spack.context.current())
...
def test_initialize_from_lockfile(tmp_path: pathlib.Path, filename):
    ...
    ev.initialize_environment_dir(env_dir, init_file, config=spack.config.CONFIG)
...
"""
    ...
    ev.create_in_dir(env_dir, init_file, ctx=spack.context.current())
...
"""
    ...
    with pytest.raises(ev.SpackEnvironmentError, match="You need to use a newer Spack version."):
        ev.Environment(env_dir, ctx=spack.context.current())
...
"""
    ...
    with ev.Environment(manifest.parent, ctx=spack.context.current()):
...
"""
    ...
    with ev.Environment(manifest.parent, ctx=spack.context.current()) as e:
        if expected_raise:
...
"""
    ...
    with pytest.raises(UndefinedReferenceError, match=r"which is not defined"):
        _ = ev.Environment(env_path, ctx=spack.context.current())
...
def test_concretize_is_noop_for_concretized_namespaced_root(mutable_mock_env_path):
    ...
    env = ev.Environment(mutable_mock_env_path, ctx=spack.context.current())
    with env:
        env.concretize()
        env.write()
    ...
    reread = ev.Environment(mutable_mock_env_path, ctx=spack.context.current())
    with reread:
...
def test_env_view_on_empty_dir_is_fine(tmp_path: pathlib.Path, config, temporary_store):
    ...
    view_dir.mkdir()
    env = ev.create_in_dir(tmp_path, with_view="view", ctx=spack.context.current())
    env.add("mpileaks")
    env.concretize()
...
def test_environment_from_name_or_dir(mutable_mock_env_path):
    test_env = ev.create("test", ctx=spack.context.current())

    name_env = ev.environment_from_name_or_dir(test_env.name, ctx=spack.context.current())
    assert name_env.name == test_env.name
    assert name_env.path == test_env.path

    dir_env = ev.environment_from_name_or_dir(test_env.path, ctx=spack.context.current())
    assert dir_env.name == test_env.name
    assert dir_env.path == test_env.path

    nested_test_env = ev.create("group/test", ctx=spack.context.current())

    nested_name_env = ev.environment_from_name_or_dir(
        nested_test_env.name, ctx=spack.context.current()
    )
    ...
    nested_dir_env = ev.environment_from_name_or_dir(
        nested_test_env.path, ctx=spack.context.current()
    )
    ...
    with pytest.raises(ev.SpackEnvironmentError, match="no such environment"):
        _ = ev.environment_from_name_or_dir("fake-env", ctx=spack.context.current())
...
def test_all_environment_names_ignores_env_contents(mutable_mock_env_path):
    ...
    ev.create("group/nested", ctx=spack.context.current())
    ...
    assert ev.all_environment_names(spack.config.CONFIG) == ["group/nested", "test"]
...
def test_all_environment_names_handles_symlink_cycles(mutable_mock_env_path):
    ...
    assert ev.all_environment_names(spack.config.CONFIG) == ["group/nested"]
...
def test_all_environment_names_follows_symlinked_envs(mutable_mock_env_path, tmp_path):
    ...
    assert ev.all_environment_names(spack.config.CONFIG) == ["tracked"]
...
def test_cannot_create_env_nested_in_another_env(mutable_mock_env_path):
    ...
    ev.create("outer", ctx=spack.context.current())
    with pytest.raises(ev.SpackEnvironmentError, match="inside existing environment 'outer'"):
        ev.create("outer/inner", ctx=spack.context.current())
...
def test_cannot_create_env_above_another_env(mutable_mock_env_path):
    ...
    ev.create("group/inner", ctx=spack.context.current())
    with pytest.raises(ev.SpackEnvironmentError, match="would contain existing environment"):
        ev.create("group", ctx=spack.context.current())
...
"""
    ...
    e = ev.Environment(env_path, ctx=spack.context.current())
    with e.manifest.use_config(spack.context.current()):
...
"""
    ...
    with ev.Environment(base, ctx=spack.context.current()) as e:
        e.concretize()
...
"""
    ...
    with ev.Environment(derived, ctx=spack.context.current()) as e:
        e.concretize()
...
class TestEnvironmentGroups:
    ...
    """
        ...
        e = ev.Environment(manifest.manifest_dir, ctx=spack.context.current())
...
"""
        ...
        with ev.Environment(manifest.manifest_dir, ctx=spack.context.current()) as e:
            e.concretize()
...
"""
    ...
    def test_from_lockfile_preserves_groups(self, tmp_path):
        ...
        manifest = EnvironmentManifestFile.from_lockfile(tmp_path, spack.config.CONFIG)
...
def test_env_substitution_reaches_the_unwrapped_configuration(
    ...
    """
    env = ev.create("test_env_path_through_singleton", ctx=spack.context.current())
    monkeypatch.setattr(spack.config, "CONFIG", Singleton(lambda: mutable_config))

    with ev.read("test_env_path_through_singleton", ctx=spack.context.current()):
...
class TestLockfileWrites:
    ...
    def test_unchanged_lockfile_is_not_rewritten(self, lockfile, monkeypatch):
        ...
        with ev.Environment(lockfile.parent, ctx=spack.context.current()) as e:
            e.write()
