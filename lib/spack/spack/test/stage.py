...
class TestStage:
    ...
    def test_resolve_paths(self, monkeypatch, config, ctx: SpackContext):
        """Test _resolve_paths."""
        ...
        canonicalized_tempdir = canonicalize_path("$tempdir", config=ctx.config)
        ...
        res_paths = [canonicalize_path(p, config=ctx.config) for p in paths]
        if temp_has_user:
