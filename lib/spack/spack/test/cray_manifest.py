...
def test_generate_specs_from_manifest(generate_openmpi_entries):
    ...
    """
    specs = entries_to_specs(generate_openmpi_entries, ctx=spack.context.current())
...
def test_translate_cray_platform_to_linux(monkeypatch, _common_compiler):
    ...
    (spec,) = entries_to_specs([spec_json], ctx=spack.context.current()).values()
...
def test_failed_translate_compiler_name(_common_arch):
    ...
    with pytest.raises(spack.compilers.config.UnknownCompilerError):
        entries_to_specs([spec_json], ctx=spack.context.current())
...
def test_read_cray_manifest(temporary_store, manifest_file):
    ...
    """
    spack.cray_manifest.read(str(manifest_file), True, ctx=spack.context.current())
    ...
    concretized_spec = spack.concretize.concretize_one(
        "depends-on-openmpi ^/openmpifakehasha", spack.context.current()
    )
...
"""
    ...
    spack.cray_manifest.read(str(manifest), True, ctx=spack.context.current())
...
"""
    ...
    with pytest.raises(spack.cray_manifest.ManifestValidationError) as e:
        spack.cray_manifest.read(invalid_json_path, True, ctx=spack.context.current())
    str(e)
...
"""
    ...
    with pytest.raises(spack.cray_manifest.ManifestValidationError) as e:
        spack.cray_manifest.read(invalid_schema_path, True, ctx=spack.context.current())
...
def test_find_external_nonempty_default_manifest_dir(
    ...
    spack.cmd.external._collect_and_consume_cray_manifest_files(
        spack.context.current(), ignore_default_dir=False
    )
...
def test_reusable_externals_cray_manifest(temporary_store, mutable_config, manifest_file):
    ...
    spack.cray_manifest.read(
        path=str(manifest_file), apply_updates=True, ctx=spack.context.current()
    )
