...
def _concretize_with_reuse(*, root_str, reused_str):
    reused_spec = spack.concretize.concretize_one(reused_str, spack.context.current())
...
def test_correct_gcc_runtime_is_injected_as_dependency(runtime_repo):
    s = spack.concretize.concretize_one(
        "pkg-a%gcc@10.2.1 ^pkg-b%gcc@9.4.0", spack.context.current()
    )
    a, b = s["pkg-a"], s["pkg-b"]
...
def test_external_nodes_do_not_have_runtimes(
    ...
    s = spack.concretize.concretize_one("pkg-a%gcc@10.2.1", spack.context.current())
...
def test_runtimes_can_be_concretized_as_standalone(runtime_repo):
    ...
    gcc_runtime = spack.concretize.concretize_one("gcc-runtime", spack.context.current())
...
def test_multiple_intel_oneapi_compilers_versions(mutable_config, runtime_repo):
    ...
    pkga_v1 = spack.concretize.concretize_one(
        "pkg-a %c,cxx=intel-oneapi-compilers@1.0", spack.context.current()
    )
    ...
    pkga_v2 = spack.concretize.concretize_one(
        "pkg-a %c,cxx=intel-oneapi-compilers@2.0", spack.context.current()
    )
