...
def test_config_deprecated_with_old_style_version_deprecation(
    mock_packages, mutable_config, ctx: SpackContext
):
    ...
    with mutable_config.override("config:deprecated", True):
        assert concretize_one("deprecated-old-style@1.0", ctx).satisfies("@1.0")

    with pytest.raises(UnsatisfiableSpecError, match="unspecified"):
        concretize_one("deprecated-old-style@1.0", ctx)


def test_version_deprecated_true_prefers_non_deprecated(config, mock_packages, ctx: SpackContext):
    """Solver picks @0.9 (non-deprecated) over @1.0 (deprecated) by default."""
    spec = concretize_one("deprecated-old-style", ctx)
...
def test_allowed_deprecation_concretizes_without_warning(
    mock_packages, concretize_scope, packages_yaml_write, recwarn, ctx: SpackContext
):
...
packages:
  all:
    deprecation:
      ...
      - severity: critical
""")
    spec = concretize_one("deprecated-with-reason@2.0", ctx)
...
def test_per_package_selector_blocks_a_higher_severity(
    mock_packages, concretize_scope, packages_yaml_write, ctx: SpackContext
):
...
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-reason@2.0", ctx)
...
def test_coexistence_old_and_new_deprecation(mock_packages, mutable_config, ctx: SpackContext):
    ...
    spec = concretize_one("deprecated-dual", ctx)
    assert spec.satisfies("@2.0")
    ...
    with mutable_config.override("packages:all:deprecation:allow", [{"severity": "critical"}]):
        assert concretize_one("deprecated-dual@1.0", ctx).satisfies("@1.0")
...
def test_label_selector_skips_deprecation(
    mock_packages, concretize_scope, packages_yaml_write, ctx: SpackContext
):
...
""")
    assert concretize_one("deprecated-with-labels@3.0", ctx).satisfies("@3.0")
...
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-labels@2.0", ctx)
...
""")
    assert concretize_one("deprecated-with-labels@2.0", ctx).satisfies("@2.0")
...
""")
    ...
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-labels@3.0", ctx)
...
""")
    ...
    assert concretize_one("deprecated-with-reason@1.0", ctx).satisfies("@1.0")
...
""")
    ...
    assert concretize_one("deprecated-with-reason@2.0", ctx).satisfies("@2.0")

    # reason=rename is matched by no selector, so it stays disallowed
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-reason@1.0", ctx)
...
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-dual@1.0", ctx)
...
def test_legacy_config_deprecated_flag_warns(mock_packages, mutable_config, ctx: SpackContext):
...
def lib_built_with_deprecated_tool(mutable_config, temporary_store, ctx: SpackContext):
    ...
    """
    with mutable_config.override("packages:all:deprecation:allow", [{"severity": "critical"}]):
        spec = concretize_one("deprecated-tool-lib ^deprecated-tool@1.0", ctx)
    assert spec["deprecated-tool"].satisfies("@1.0")
    for node in spec.traverse():
        temporary_store.layout.create_install_directory(node, ctx.config)
        temporary_store.db.add(node, explicit=node.name == spec.name)
    return spec


def test_runtime_scope_does_not_build_with_a_deprecated_tool(
    mock_packages, mutable_config, lib_built_with_deprecated_tool, ctx: SpackContext
):
    ...
    client = concretize_one("deprecated-tool-client", ctx)
...
""")
    ...
    assert concretize_one("deprecated-tool-client", ctx)["deprecated-tool"].satisfies("@1.0")
...
""")
    ...
    assert concretize_one("deprecated-tool-client", ctx)["deprecated-tool"].satisfies("@2.0")
...
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated spec"):
        concretize_one("deprecated-tool-client %deprecated-tool@1.0", ctx)
...
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated spec"):
        concretize_one("deprecated-tool-client", ctx)


def test_directive_message_is_reported_on_refusal(
    mock_packages, mutable_config, ctx: SpackContext
):
    ...
    with pytest.raises(UnsatisfiableSpecError, match="use @2.0, which is maintained"):
        concretize_one("deprecated-with-message@1.0", ctx)


def test_directive_without_message_reports_only_the_policy(
    mock_packages, mutable_config, ctx: SpackContext
):
...
def test_every_message_behind_one_error_term_is_reported(
    mock_packages, mutable_config, ctx: SpackContext
):
    ...
    """
    with pytest.raises(UnsatisfiableSpecError) as exc_info:
        concretize_one("deprecated-with-message@0.9", ctx)
...
def test_selector_attributes_are_conjunctive(
    mock_packages, concretize_scope, packages_yaml_write, ctx: SpackContext
):
...
def test_one_matching_selector_is_enough(
    mock_packages, concretize_scope, packages_yaml_write, ctx: SpackContext
):
...
"""
    ...
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-labels@1.0", ctx)
