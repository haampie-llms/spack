...
def test_version_style_hidden(config, mock_packages):
    ...
    s = spack.concretize.concretize_one("mpileaks@2.3", spack.context.current())
...
def test_single_variant_style_hidden(config, mock_packages):
    ...
    s = spack.concretize.concretize_one("mpileaks+debug", spack.context.current())
...
def test_abstract_spec_str_roundtrips_namespace(config, mock_packages):
    ...
    concrete = spack.concretize.concretize_one("mpileaks", spack.context.current())
...
def test_variants_of_concrete_spec_abbreviate_patches(config, mock_packages):
    ...
    concrete = spack.concretize.concretize_one("patch", spack.context.current())
