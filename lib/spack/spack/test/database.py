...
def test_010_all_install_sanity(database, ctx: SpackContext):
...
def test_025_reindex(mutable_database, ctx: SpackContext):
    """Make sure reindex works and ref counts are valid."""
    ctx.store.reindex()
    _check_db_sanity(mutable_database)


def test_026_reindex_after_deprecate(mutable_database, ctx: SpackContext):
...
def test_115_reindex_with_packages_not_in_repo(
    mutable_database, repo_builder: RepoBuilder, ctx: SpackContext
):
...
def test_failed_spec_path_error(mutable_database, ctx: SpackContext):
...
@pytest.mark.db
def test_clear_failure_keep(mutable_database, monkeypatch, capfd, ctx: SpackContext):
...
@pytest.mark.db
def test_clear_failure_forced(mutable_database, monkeypatch, capfd, ctx: SpackContext):
...
def test_mark_failed(
    mutable_database, monkeypatch, tmp_path: pathlib.Path, capfd, ctx: SpackContext
):
...
@pytest.mark.db
def test_prefix_failed(mutable_database, monkeypatch, ctx: SpackContext):
...
def test_prefix_write_lock_error(mutable_database, monkeypatch, ctx: SpackContext):
