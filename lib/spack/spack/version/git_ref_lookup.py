...
def resolve_git_version(pkg_name: str, version: GitVersion) -> None:
    ...
    :func:`resolve_git_versions`, and ``packages.yaml`` version preferences directly.

    Raises:
        VersionLookupError: if the package has no ``git`` attribute or the ref is unknown.
    """
...
def resolve_git_versions(spec: "spack.spec.Spec") -> "spack.spec.Spec":
    ...
    for node in spec.traverse():
        ...
        for v in node.versions:
            if isinstance(v, GitVersion):
                resolve_git_version(node.fullname, v)
    return spec
