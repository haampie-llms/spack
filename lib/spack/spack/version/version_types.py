...
class GitVersion(ConcreteVersion):
    ...
    is assigned by :func:`spack.version.git_ref_lookup.resolve_git_versions`, which happens
    ...
    def __init__(self, string: str):
        ...
        if "=" in normalized_string:
            ...
            # Assigned later by spack.version.git_ref_lookup.resolve_git_versions, since we
    ...
    def ref_version(self) -> StandardVersion:
        ...
        Raises:
            VersionLookupError: if no version has been assigned yet. Assignment happens once
                at concretization, see :func:`spack.version.git_ref_lookup.resolve_git_versions`.
        """
    ...
    def intersection(self, other: VersionType) -> VersionType:
        if isinstance(other, GitVersion):
            if not self.intersects(other):
                return VersionList()
            # Prefer the side that carries an assigned version
